		...
		<sx:div label="Inicio">
			...
			<ol>
				...
				<s:iterator value="#session.comments" var="comment">
					...
					<s:date  name="%{new java.util.Date()}" format="dd/MM/yyyy" var="today"/>
    				<s:date  name="#comment.commentDate" format="dd/MM/yyyy" var="commentDate"/>
    				<s:date  name="%{new java.util.Date(new java.util.Date().getTime() - 60*60*24*1000)}" format="dd/MM/yyyy"  var="yesterday"/>
					...
					<s:date name="#comment.commentDate" format="HH:mm:ss" /> h
					<s:if
							test="#session.user.idUser == #comment.user.idUser">
							<a href="editComment">Modificar</a> | <s:a href="deleteComment?idComment=%{#comment.idComment}">Eliminar</s:a>
						</s:if></li>
					<br/>
				</s:iterator>
			</ol>
